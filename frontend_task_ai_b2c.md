...
| Поле | Тип | Обязательное | Описание |
|------|-----|:---:|----------|
| `stage_key` | string (select или input) | ✅ | Ключ этапа — уникальный идентификатор страницы |
| `title` | string | ✅ | Человекочитаемое название этапа |
...
### Доступные значения `stage_key`

| Ключ | Страница |
|------|----------|
| `PFP1` | Главная / Начало |
| `chooseTarget` | Выбор целей |
| `riskProfile` | Риск-профилирование |
| `financialReserve` | Финансовый резерв |
| `assets` | Активы и пассивы |
| `result` | Результат / Финплан |

> [!IMPORTANT]
> `stage_key` уникален в рамках проекта. Если пытаются создать дубликат — бэк вернёт 400.

### UI

- Таблица с колонками: Ключ этапа, Название, Статус
- Кнопка "Создать этап"
- Поле `stage_key` лучше сделать **выпадающим списком** из значений выше
